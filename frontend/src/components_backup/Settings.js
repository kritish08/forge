      ...
      <div className="px-6 pt-5">
        ...
        {activeTab === "notifications" && (
          <div className="space-y-4">
            ...
            {/* Timezone & Scheduling */}
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              <h3 className="font-bold font-chivo text-gray-900 mb-1">🌍 Timezone & Timing</h3>
              <p className="text-xs text-gray-400 font-manrope mb-4 leading-relaxed">
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-sm font-bold text-gray-900 font-manrope block mb-2">Timezone</label>
                  <select 
                    value={user?.timezone || "UTC"}
                    onChange={(e) => api.put("/user/settings", { timezone: e.target.value }).then(refreshUser).then(() => toast.success("Timezone updated!"))}
                    className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm font-manrope focus:outline-none focus:ring-2 focus:ring-orange-500"
                  >
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-sm font-bold text-gray-900 font-manrope block">Notification Schedules</label>
                    <button 
                      onClick={() => {
                  ...
                  {(user?.notification_rules || [{ time: "20:00" }]).map((rule, idx) => (
                    <div key={idx} className="flex gap-2 items-center">
                      <div className="flex-1 bg-gray-50 border border-gray-200 rounded-xl flex items-center px-4 overflow-hidden focus-within:ring-2 focus-within:ring-orange-500">
                        <span className="text-gray-500 text-sm font-bold font-chivo mr-2">Time:</span>
                        <input 
                          type="time" 
                  ...
                  {user?.notification_rules?.length === 0 && (
                    <p className="text-sm text-gray-500 italic py-2">No schedules set. You will not receive any daily reminders.</p>
                  )}
